...
def format_token_count(used: int, max_tokens: int | None = None) -> str:
    """格式化 token 数量，如 "12.3k" 或 "12.3k/80k (15%)"。
    ...
    if max_tokens and max_tokens > 0:
        pct = int(used / max_tokens * 100)
        result = f"{_fmt(used)}/{_fmt(max_tokens)} ({pct}%)"
    return result
