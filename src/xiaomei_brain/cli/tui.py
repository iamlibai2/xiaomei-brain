...
def _ensure_deps() -> None:
    ...
    try:
        ...
        _HSplit = _H
        _Window = _W
        _FormattedTextControl = _F
        _ConditionalContainer = _CC
        _Condition = _Cd
        _Dimension = _D
        _TextArea = _TA
        _KeyBindings = _KB
...
async def _login(ws) -> tuple[str, str]:
    ...
    connect_req = {
        ...
        "id": "connect-1",
        "method": "connect",
        "params": {"token": token, "client": "ws-cli", "user_id": uid},
    }
...
def _build_app(ws, agent_name: str, user_id: str):
    state = _get_state()
    state.ws = ws
    ...
    def _input_height() -> int:
        try:
            ...
            prompt_width = max(2, get_cwidth(f"[{_get_state().user_id}]> "))
            try:
    ...
    status_bar = _ConditionalContainer(
        _Window(
            content=_FormattedTextControl(_get_status_bar_fragments),
            height=1,
            wrap_lines=False,
        ),
...
def _build_status_bar_fragments() -> list[tuple[str, str]]:
    ...
    if s.agent_name:
        parts.append(("class:status-bar-strong", f" {s.agent_name} "))
    ...
    parts.append(("class:status-bar-dim", "\u2502"))
    parts.append(("", f" {s.msg_count} msg "))

    if s.latency > 0:
        parts.append(("class:status-bar-dim", "\u2502"))
        parts.append(("", f" {s.latency}ms "))

    ts = datetime.now().strftime("%H:%M")
    trailing = f"Ctrl-C 中断/退出  /help 帮助  {ts} "
