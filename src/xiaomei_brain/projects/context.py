...
from .models import ProjectActor, ProjectActorType, ProjectStatus
from .service import ProjectService


def render_project_context(agent: Any, *, limit: int = 6) -> str:
    """Render durable work facts without exposing paths or private scopes."""
    ...
    try:
        active_projects = service.list_for_actor(
            actor=actor, status=ProjectStatus.ACTIVE, limit=limit,
        )
    ...
    if active_id:
        try:
            ...
            current = None
    if current is None and not active_projects:
        return ""
    ...
    others = [
        ...
        if current is None or project.id != current.id
    ]
    if others:
        lines.append("<other_active>")
        for project in others[:limit]:
            summary = project.progress_summary or project.summary
            lines.append(
                f'- id="{escape(project.id)}" type="{escape(project.project_type)}" '
                f'name="{escape(project.name[:160])}": {escape(summary[:300])}',
            )
        lines.append("</other_active>")
    lines.append(
