...
Use `write_document` validation and any available local PPTX rendering workflow to check overflow, overlap, truncation, garbled text, low contrast, missing page numbers, and missing sources. Review key pages individually when rendering is available; never claim visual verification from structural validation alone.
