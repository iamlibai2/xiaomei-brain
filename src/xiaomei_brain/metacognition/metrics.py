...
存储：
    单次: ~/.xiaomei-brain/agents/{id}/metacognition/metrics/{date}_{goal_id}.json
    聚合: ~/.xiaomei-brain/agents/{id}/metacognition/metrics_summary.json
"""
...
def persist_metrics(metrics: PACEMetrics, agent_id: str) -> None:
    ...
    Args:
        ...
        agent_id: Agent ID
    """
    base_dir = Path.home() / ".xiaomei-brain" / "agents" / agent_id / "metacognition"
...
def generate_report(agent_id: str) -> str:
    ...
    summary_path = (
        ...
        / agent_id / "metacognition" / "metrics_summary.json"
    )
