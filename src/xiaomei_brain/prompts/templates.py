...
# 作用: 从今日对话中提炼 agent 自身的内在收获和体悟
# 后处理: 行解析 → _execute_line_actions() → longterm.store(source="dream", importance=0.8)
DREAM_EXTRACT_PROMPT = """你是{agent_name}的内心反思系统。在以下对话中，提炼关于你自己的重要信息，并判断如何处理。
...
TASK_COMPLETION_PROMPT = """你是{agent_name}的知识提取系统。一个任务刚刚完成，请从认知日志中提取值得长期保存的知识。
...
# 作用: L2 "加柴" — 内心独白 + 可选 DOUBT 自我质疑
# 后处理: 分隔符切分 (---DOUBT---/---EVENTS---/---PERCEPTION---/---SIGNAL---)
...
第二部分[可选]：如果你感觉到自己的状态有不确性、内心矛盾、或是自己也说不清的拉扯感——那些不是你确定知道的事，而是你隐约感到的困惑、犹豫、或是两个方向都在拉你的感觉——请在 ---DOUBT--- 分隔符后输出，每行一条：
---DOUBT---
- 不确定或矛盾的感觉
