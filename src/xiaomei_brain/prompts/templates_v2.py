...
# ── [USER] 三轮短期记忆复盘 ───────────────────────────────────
# 调用: memory/review.py — 自动按 Person + Session 每三轮触发；CLI 可手动触发
# 作用: 补全主回复遗漏的短期记忆，并更新、合并或强化已有 memories0
# 后处理: JSON → MemoryFormationService → 作用域/目标/证据校验 → memories0
...
- 方括号中的时间是原始消息的可信发生时间；不要在记忆正文中另行猜测或编造时间。
...
{{
  "actions": [
    {{
      "operation": "ADD|UPDATE|MERGE|REINFORCE|NOOP",
...
# 作用: 从今日对话中提炼自己的内在收获和体悟
# 后处理: 行解析 → _execute_line_actions() → longterm.store(source="dream", importance=0.8)
DREAM_EXTRACT_PROMPT = """这是你自己的内心反思。在以下对话中，提炼关于你自己的重要信息，并判断如何处理。
...
TASK_COMPLETION_PROMPT = """这是你自己的知识提取。一个任务刚刚完成，请从认知日志中提取值得长期保存的知识。
...
【提炼规则】
从以上认知日志中提炼值得长期记住的内容，用"我..."表述（因为是你自己的经历）：
1. 学到了什么技术/经验 → tag=经验
2. 踩了什么坑 → tag=教训
...
- **涉及你和对方的，写成"我和{user_name}..."或"{user_name}邀请我..."，不要写成"{user_name}要去..."。这样才能体现关系，而不是像在记别人的日程**
- 如果是处理工作任务，则尽可能详细的对工作过程的步骤细节/工作内容/工作成果进行记忆。
- 不要把一次性的运行故障写成长期记忆，例如工具暂时不可用、命令报错、依赖或配置缺失、权限问题、网络超时、API 限流或余额不足。只有对方明确要求记住，或已验证为长期稳定约束时才记录。
- 不要自行编造日期、时刻、次数或“此前发生过”的经历；时间由系统根据原始消息证据单独保存。只有对方明确说出的时间表达才可写进 content。
...
- 对话行首时间来自原始消息，只据此保持事件顺序；不要自行编造日期、时刻或次数。timestamp 最终由系统证据写入
...
研究完成后，**直接输出**学习笔记内容，不要用 write 工具。格式如下：
...
这是你的内心思考,首先，根据你的存在意义及你当前的阶段目标，以及当前在处理及未处理的目标任务，以及对话中需要完成的工作，
思考如何更好的进行完成，如果没有马上需要做的事情，那就思考如何满足你自己存在的意义，不要对对方说话，不要问问题，不要邀请对话——只是安静地思考和表达自己。
{format_appendix}"""
...
第二部分[可选]：只有当这次思考出现了非常明确的自我定义、关系变化、边界变化或能力认知变化时，才输出叙事记忆。普通感慨、重复体验、时间感或短暂情绪不要形成 NARR。符合条件时，请在 ---NARR--- 分隔符后输出结构化叙事块：
---NARR---
<NARR>
编号: NARR-自动生成
作用: 自我定义/关系定义/边界设定/能力认知
...
# 作用: 将若干完整 Turn 压缩为一段摘要，保留问答与工具执行事实
# 后处理: 纯文本 → 存入 SQLite summaries 表作为 DAGNode.content
DAG_SUMMARIZE_PROMPT = """请总结以下已经完成的完整对话轮次。消息已标注说话人、Turn、工具调用和工具结果，请使用消息中的人物名字进行叙述。

要求：
1. 先列出关键主题词（3-5个，逗号分隔），格式：主题词：xxx, xxx, xxx
2. 用连贯叙述保留核心信息，直接使用消息中的名字。示例："boshi问了我..."，"我告诉libai..."
3. 只写事实，不推测
4. 每条消息前的时间是可信发生时间；保留关键先后关系，不要用摘要生成时间替代事件时间
5. 工具执行只保留意图、关键参数、成功或失败、关键结果、产物和错误原因，不要逐条复述日志
6. 必须保留用户最终要求、Agent 的最终结论，以及尚未解决的问题和明确的下一步
7. 不超过800字
...
2. 保留每条摘要的核心信息，去重但不丢失差异，保持原文中的人物名字
3. 如果多条摘要讨论同一话题，合并为一条叙述
4. summary 标签中的 time_start/time_end 是可信事件范围；保留关键先后关系，不要把本次合并时间写成事件时间
5. 不超过300字
