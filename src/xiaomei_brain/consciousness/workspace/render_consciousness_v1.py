"""意识渲染 — 将 SelfImage 各模块数据渲染为 LLM 可读的文本片段。

每个 _render_* 函数输入 SelfImage，输出 list[str]。
不做任何外部系统调用或 Side Effect。

Usage:
    from .render_consciousness import (
        _render_header, _render_being, _render_body, _render_mind,
        _render_memory, _render_milestones, _render_self_trajectory,
        _render_inner_voice, _render_desk, _render_environment,
        _render_history, _render_experience, _render_experience_timeline,
        _render_project_map, _render_intent, _render_pace_reflections,
        _render_essence, _render_being_legacy,
    )
"""
...
def _render_milestones(si) -> list[str]:
