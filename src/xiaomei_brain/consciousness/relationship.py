...
事件：
- on_user_message()   → depth + 0.02, interaction_count + 1
...
class RelationshipStorage(SQLiteStore):
    ...
    def save(self, user_id: str, depth: float, trust: float,
             interaction_count: int, last_interaction_time: float,
             last_decay_time: float) -> None:
        """保存关系数据（INSERT OR REPLACE）。"""
        conn = self._get_conn()
        conn.execute(
            """INSERT OR REPLACE INTO relationships
               (user_id, depth, trust, interaction_count, last_interaction_time, last_decay_time)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (user_id, depth, trust, interaction_count, last_interaction_time, last_decay_time),
        )
        conn.commit()
...
class RelationshipEngine:
    ...
    def on_social_signal(self, signal_type: str, intensity: float) -> None:
        ...
        logger.info("[Relationship] on_social_signal: %s → trust=%.2f (delta=%.3f)",
                    signal_type, self.trust, scaled_delta)
