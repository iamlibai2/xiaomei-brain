...
阶段 D3/D4 收紧为可实施的本地闭环：

- D3 支持安装、启用、停用、覆盖更新和卸载；
- 更新时同步所有本地 Agent 的包引用，界面只展示当前版本；
- 不建立多版本选择和回滚产品，问题包通过重新安装另一个 `.xmcap` 处理；
- D4 提供 `capability pack/inspect` CLI，将开发好的源目录导出为可检查、可安装的确定性归档；
- 由实施人员向客户交付并安装本地能力包。

本阶段不做在线发布、能力市场、企业能力源、自动下载、发布者签名、自动安装 Python/Node/FFmpeg 依赖，也不做开发者脚手架和复杂项目模板。
