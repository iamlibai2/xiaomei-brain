...
PDF 插件复用同一 `write_document` 和执行现场，支持：

- A4/Letter、横向/纵向和自定义页边距；
- 统一中文字体、颜色、字号和行距；
- 标题、段落、列表、表格、图片、引用、留白和分页；
- 页眉、页脚、页码以及标题、作者等文档属性；
- 受控图片附件和 workspace 图片；
- 保存后通过 pypdf 和现有 PDF extractor 验收页数与文本层。

第一版只从结构化内容创建新 PDF，不原地修改已有 PDF。

## 后续顺序

1. 可选视觉验收：存在 Poppler、LibreOffice 等渲染后端时生成预览，不作为基础安装的硬依赖。
