# ARCHITECTURE.md — xiaomei-brain 架构总览

> 面向开发者。读完本文，你可以在半小时内找到想改的代码。
...
└─────────────────┘  └─────────────────┘  └─────────────────┘
          ...
          └────────────────────┼────────────────────┘
               ...
               ┌─────────────────────────────┐
               │  Metacognition（元认知层）     │
               │  SocialPerception  │  PACE   │
               └─────────────────────────────┘
```

**数据流**：消息进入 → Gateway 路由 → Living 排队 → ContextAssembler 组装上下文 → Agent.stream() 调用 LLM → 记忆提取 → 回复经 Router 分发给各 Channel。
...
| 目录 | 职责 | 核心文件 |
|------|------|---------|
| `agent/` | Agent 核心：ReAct 循环、工具调用、流式输出 | `core.py`（stream 主循环） |
| `memory/` | 记忆系统：存储、摘要、召回、SelfModel | `longterm.py`（长期记忆）、`dag.py`（DAG 摘要）、`self_model.py`（身份模型） |
| `consciousness/` | 意识生命周期：心跳、加柴、梦境 | `conscious_living.py`（主循环）、`self_image_proxy.py`（意识火焰中枢） |
| `drive/` | 边缘系统：情绪、激素、欲望、激励 | `engine.py`（Drive 引擎） |
| `purpose/` | 前额叶：目标管理和意图理解 | `purpose_engine.py`（目标引擎）、`intent.py`（意图分析） |
| `metacognition/` | 元认知：自我监督与反省 | `social_perception.py`（社交感知，被动收集） |
| `gateway/` | 网关：消息路由、peer 管理 | `router.py`（Router） |
| `plugins/channels/` | 消息渠道插件 | `cli/`、`feishu/`、`dingtalk/` |
| `plugins/tools/` | 工具插件 | `tts_minimax/`、`web_search_baidu/`、`image_minimax/` |
| `plugins/providers/` | LLM Provider 插件 | `deepseek/`、`anthropic/` |
| `tools/builtin/` | 内置工具 | `shell.py`、`file_ops.py`、`memory.py` |
| `cli/` | CLI 命令入口 | `__init__.py`（路由表）、`run.py`（启动）、`setup.py`（初始化向导） |
| `llm/` | LLM 客户端和传输层 | `client.py` |
| `config/` | 配置系统 | `agent_config.py`（AgentConfig） |
| `body/` | 具身层：设备管理 | `device/` |
| `tui/` / `tui_v2/` | 终端 UI | TUI 界面（开发中） |
...
python -m xiaomei_brain run xiaomei
    ...
    __main__.py → cli/main()
           │
           ├─ _bootstrap_xiaomei()        # 首次：创建预置 identity.md + config.yaml
           │
           └─ cli/run.py:cmd_run()
                    │
                    ...
                    └─ living.run(interactive=True)       # 纯同步主循环
                              │
                              └─ while not stopped: tick() → _check_conversation()
```
...
用户输入 (CLI / 飞书 / 钉钉)
           │
           ▼
    Channel adapter → gateway/router.py
           │
           ├─ peer_id → user_id 映射
           └─ living.put_message(text, user_id, session_id)
                    │
                    ▼
    Living 主循环 tick() → _check_conversation()
           │
           ...
           │      ├─ Tool calling (shell / file / memory / web_search / ...)
           │      └─ Stream 输出
           │
           ├─ 3. Memory Extractor                # 记忆提取
           │      ├─ immediate: 关键词触发 → 立即提取
           │      ├─ periodic: 每 10 轮对话 → 批量总结
           │      └─ dream: 空闲 >5min → 深度反思
           │
...
- **可解释性**：算法规则（衰减系数、阈值）透明可调试，LLM 判断是黑盒
- **稳定性**：不会因为 LLM 随机性导致情绪剧烈波动
- LLM 只负责**识别事件**（"用户表扬我了"→ EVENT_PRAISE），代码决定**数值变化**（多巴胺 +0.1）
...
- "用户喜欢吃辣"vs"用户告诉我他喜欢吃辣"——前者是事实，后者是经历
...
| 你想了解 | 从这里开始 |
|---------|-----------|
| **聊天怎么发生** | `agent/core.py:192` — `stream()` 方法，ReAct 循环 |
| **消息怎么处理** | `consciousness/conscious_living.py:84` — `__init__` 和 `_check_conversation()` |
...
| **CLI 命令怎么添加** | `cli/__init__.py:71` — `main()` 路由表 |
| **渠道怎么接入** | `plugins/channels/feishu/adapter.py` — 飞书 adapter 完整示例 |
...
|---|------|
| 语言 | Python 3.11+ |
| LLM 客户端 | OpenAI 兼容协议（支持 DeepSeek/GLM/MiniMax/豆包） |
| 向量数据库 | LanceDB（本地嵌入式） |
| Embedding | BAAI/bge-m3（1024 维，中文优化） |
| 对话存储 | SQLite + FTS5 全文搜索 |
| 消息渠道 | 飞书 SDK / 钉钉 Stream / WebSocket |
| 终端 UI | Rich |
| 包管理 | uv / pip |
